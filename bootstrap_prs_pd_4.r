...
pud_prs=c(AP,quantile(r,c(0.025,0.975)))
print("finished pud")

# menarche
library(RNOmni)
df = df %>% filter(!is.na(`Age when periods started (menarche).0.0`))
df$menarche=rankNorm(df$`Age when periods started (menarche).0.0`)
...
print("finished menarche")



prs_summary = data.frame(rbind(pud_prs,menarche_prs))
write_csv(prs_summary,"prs_APs_4.csv")
